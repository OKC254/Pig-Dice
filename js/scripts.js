...
async function computerVeteran(shepherd,human){
	...
	console.log("Shepherd rolled a: "  + roll2 );	
	$("#player-2-roll").text('Shepherd rolled a: '  + roll2 );
	
	if (roll2 < 2) {
		var score2 = shepherd.score;
		playerTwoToOne();
		$("#player-2-session").text("Shepherd's session score is: 0 " );
		$("#player-2-score").text("Shepherd's total score is: " + score2 );
			
	} 
	else if(session2<((shepherd.winscore-shepherd.score)/(shepherd.winscore-human.score))*15 && shepherd.sessionRolls<=3){
		$("#player-2-session").text("Shepherd's session score is: " + session2);
